...
class DemoOSSM : public OSSM {
  ...
  public:
    explicit DemoOSSM() : OSSM(nullptr) {
        ...
        menu.clear();
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Simple Stroke",
                                researchAndDesireWaves, std::nullopt, -1, -1,
                                0});
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Teasing Pounding",
                                researchAndDesireHeart, std::nullopt, -1, -1,
                                1});
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Robo Stroke",
                                researchAndDesireTerminal, std::nullopt, -1, -1,
                                2});
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Half & Half",
                                researchAndDesireFaceWink, std::nullopt, -1, -1,
                                3});
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Deeper",
                                researchAndDesireArrowsRight, std::nullopt, -1,
                                -1, 4});
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Stop & Go",
                                researchAndDesireHourglass03, std::nullopt, -1,
                                -1, 5});
        menu.push_back(MenuItem{MenuItemE::DEVICE_MENU_ITEM, "Insist",
                                researchAndDesireItalic02, std::nullopt, -1, -1,
                                6});
    ...
    void onConnect() override {
        ...
        // Just setup the initial values and UI state without calling private
        // methods
        isFirstConnect = false;

        setMiddleLed(Colors::white, 50);
        // Enable menu button for demo mode (unlike real OSSM which starts
        // disabled)
        if (menuButton) {
    ...
    void onLeftEncoderChange(int value) override {
        ...
        ESP_LOGI("DemoOSSM", "Demo mode - left encoder changed to: %d (speed)",
                 value);
    }
    ...
    void onDeviceMenuItemSelected(int index) override {
        ...
        if (index >= 0 && index < menu.size()) {
            settings.pattern =
                static_cast<StrokePatterns>(menu[index].metaIndex);
            patternName = menu[index].name;
            ESP_LOGI("DemoOSSM", "Demo mode - pattern selected: %s",
                     patternName.c_str());
            ...
            if (patternNameDisplay) {
                ...
                ESP_LOGI("DemoOSSM", "Updated pattern name to: %s",
                         patternName.c_str());
            }
        }
    }
    ...
    bool isInDemoMode() const { return isDemoMode; }
  ...
  protected:
    ...
    std::string readString(const std::string &characteristicName) {
        ...
        if (characteristicName == "state") {
            ...
            // Use the newer add<JsonObject>() method instead of deprecated
            // createNestedObject()
