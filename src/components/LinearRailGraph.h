...
public:
    ...
    void draw() override
    {
        if (xSemaphoreTake(displayMutex, pdMS_TO_TICKS(50)) == pdTRUE)
        {
            ...
            // - stroke determines how far left from that right edge the bar extends
            
            int borderMargin = 3; // 1px border + 2px for rounded corner protection
            ...
            int fillStart = depthRightEdge - strokeExtent; // Left edge extends back by stroke amount
            int actualFillWidth = max(0, strokeExtent);
            ...
            if (lastStrokeWidth == -1)
            {
                ...
                if (actualFillWidth > 0) {
                    ...
                    tft.fillRect(screenFillStart, y + 1, actualFillWidth, height - 2, COLOR_WHITE);
                }
                
                lastStrokeWidth = currentStrokeWidth;
                lastDepthOffset = currentFillStart;
            }
            else if (lastStrokeWidth != currentStrokeWidth || lastDepthOffset != currentFillStart)
            {
                // Differential drawing to prevent flicker - only change what needs to change
                ...
                if (lastStrokeWidth > 0) {
                    // Clear left overhang (old bar extends further left than new bar)
                    if (screenOldStart < screenNewStart) {
                        int clearWidth = min(screenNewStart, screenOldEnd) - screenOldStart;
                        if (clearWidth > 0) {
                            tft.fillRect(screenOldStart, y + 1, clearWidth, height - 2, ST77XX_BLACK);
                        }
                    }
                    
                    // Clear right overhang (old bar extends further right than new bar)  
                    if (screenOldEnd > screenNewEnd) {
                        int clearStart = max(screenNewEnd, screenOldStart);
                        int clearWidth = screenOldEnd - clearStart;
                        if (clearWidth > 0) {
                            tft.fillRect(clearStart, y + 1, clearWidth, height - 2, ST77XX_BLACK);
                        }
                    }
                }
                ...
                if (actualFillWidth > 0) {
                    // Fill left extension (new bar extends further left than old bar)
                    if (screenNewStart < screenOldStart) {
                        int fillWidth = min(screenOldStart, screenNewEnd) - screenNewStart;
                        if (fillWidth > 0) {
                            tft.fillRect(screenNewStart, y + 1, fillWidth, height - 2, COLOR_WHITE);
                        }
                    }
                    
                    // Fill right extension (new bar extends further right than old bar)
                    if (screenNewEnd > screenOldEnd) {
                        int fillStart = max(screenOldEnd, screenNewStart);
                        int fillWidth = screenNewEnd - fillStart;
                        if (fillWidth > 0) {
                            tft.fillRect(fillStart, y + 1, fillWidth, height - 2, COLOR_WHITE);
                        }
                    }
                }
